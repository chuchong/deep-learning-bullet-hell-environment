...
class Vectorization:
    ...
    def get_vector(self, game_data: GameData):
        ...
        for bullet in game_data.bullets:
            ...
            if dist / (self.w + self.h) < min_dists[cur]:
                min_dists[cur] = dist / (self.w + self.h)
        # 加上密度
        ...
        # 加上相对面积
        region_area = np.zeros([4])
        region_area[0] = (self.w - center[0]) * (self.h - center[1])
        region_area[1] = (center[0]) * (self.h - center[1])
        region_area[2] = (center[0]) * center[1]
        region_area[3] = (self.w - center[0]) * center[1]
        region_area /= (self.w * self.h)

        vec = np.hstack([vec, min_dists, region_area, region_dens])
        return vec